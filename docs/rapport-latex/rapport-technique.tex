...
\toprule
\textbf{Alerte} & \textbf{Condition} & \textbf{Durée} & \textbf{Pourquoi ce seuil} \\
\midrule
Service indisponible & sonde en échec & 3 min & Six sondes : une seule peut être un aléa réseau \\
Latence élevée       & $>$ 5 s        & 5 min & Le réveil d'instance produit une latence légitime \\
Certificat TLS       & $<$ 15 jours   & 1 h   & Marge d'intervention si le renouvellement échoue \\
\bottomrule
\end{tabular}
\caption{Trois arbitrages entre détection tardive et fausse alerte}
\end{table}

Chaque seuil arbitre entre détecter trop tard et alerter pour rien. Une
alerte qui se déclenche sur un comportement attendu finit filtrée — ce qui
neutralise aussi les vraies.
